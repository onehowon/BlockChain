...
# 블록암호:운영모드
### 패딩
#### 블록 암호는 블록 단위의 암호화를 하므로 입력 길이가 정확하게 블록 크기의 배수가 되야하나, 일반적으로 평문의 크기는 블록 크기의 배수가 되진 않는다.
#### 이런 문제를 해결하고자 평문에 데이터를 붙여 크기가 블록 크기가 배수가 되도록 하는 과정을 패딩(Padding)이라고 한다.

### 비트 패딩(Bit Padding)
#### 마지막 블록에서 평문이 채우지 못하는 비트 중 최상위 비트를 1로 설정하고, 나머지는 모두 0으로 채우는 패딩 기법이다.
#### 이 방법은 평문의 크기가 정확히 블록 크기의 배수일 때, 수신자가 메세지의 일부를 패딩으로 오인하게 되는 문제로, 이를 막기 위해 비트 패딩 적용시 평문 크기가 블록 크기 배수면, 패딩으로 한 블록 추가한다.

### 바이트 패딩: ANSI X.923
#### ANSI X.923은 마지막 블록의 남는 바이트를 임의의 값으로 채우고, 마지막 바이트에 패딩의 길이를 기록하는 기법이다.

### 바이트 패딩: PKCS#7
#### PKCS(Public Key Cryptography Standard)에 소개된 패딩 기법은 추가할 패딩의 바이트 크기로 마지막 블록을 채우는 패딩 기법이다.

### ECB 모드
#### 가장 간단한 운영모드로, 블록들은 모두 같은 키로 암호화 된다.
#### 각 블록이 독립적으로 암호화되므로 여러 블록을 병렬적으로 암호화할 수 있다.
#### ECB는 다른 운영모드에 비해 암호학적 안전성이 부족하다.
![image](https://user-images.githubusercontent.com/81984723/187129400-92e5db3b-6dfa-4d89-9858-cc5627dfbc1f.png)

### ECB 모드의 취약점
#### 같은 블록은 같은 암호문으로 암호화되는 ECB 모드 특성상, 암호문에서 평문 정보를 습득하거나, 리플레이 공격을 수행하는 것이 가능하다.
